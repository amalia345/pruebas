#include.<stdio.h>

int main(int argc, char const *argv[]) //argv donde estan cuandadas los arreglos que escribes en la linea de comando
{
    int 
    printf("Argumentos en la linea de comandos: \n, %d");
    while(--argc)
        
        printf("%s\n", *argv);
        //printf("%s\n/" argv[ ++i]); //con Notacion de arreglos 


    return 0;
}


//AL final el programa crea un archivo nuevo en el que imprimi el resultado de lo anterior 
//EL contenido de un archivo lo deve de copiar en otro archivo 
//acceder desde la terminal o encontrar la manera de 
//crear mi copy.c
//micopyarch1.txt arch2.txt
